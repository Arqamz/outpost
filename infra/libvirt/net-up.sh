...
# DHCP range spans the whole node block so leases are honoured.
range_start="$(node_ip 1)"
range_end="$(node_ip "${CLUSTER_NODE_COUNT}")"
...
virsh net-dumpxml "${CLUSTER_NET_NAME}" | grep -E 'range|host mac' | sed 's/^/  /'
